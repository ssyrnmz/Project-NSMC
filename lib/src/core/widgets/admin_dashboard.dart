...
import '../../features/appointment/domain/appointment.dart';
import '../../features/appointment/presentation/views/apt_requested_screen.dart';
import '../../utils/ui/animations_transitions.dart';

class AdminDashboardBox extends StatelessWidget {
  const AdminDashboardBox({super.key, required this.appointments});
      ...
      child: Container(
        ...
        child: Row(
          ...
          children: [
            ...
            Row(
              children: [
                FaIcon(icon, color: iconColor, size: 21),
                if (onTap != null) ...[
  ...
  Widget build(BuildContext context) {
    ...
    for (Appointment apt in appointments) {
      ...
      if (apt.createdAt.year == now.year &&
          apt.createdAt.month == now.month &&
          apt.createdAt.day == now.day) {
        recentAppointments.add(apt);
      }
    }

    String newAppointments = '${recentAppointments.length}';
    String awaitingApproval = '${appointments.length}';
      ...
      child: Container(
        ...
        child: Column(
          ...
          children: [
            ...
            Text(
              'Overview of appointment requests',
              style: GoogleFonts.poppins(
                fontSize: 13,
                fontWeight: FontWeight.w400,
                color: const Color(0xFF667085), // Zus-like gray text
              ),
            ),
            const SizedBox(height: 20),

            _buildMetricCard(
              title: 'Today\'s Requested Appointments',
              value: newAppointments,
              icon: FontAwesomeIcons.circleExclamation,
              iconColor: const Color(0xFF175CD3),
              onTap: () => Navigator.of(context).push(
                transitionAnimation(page: const AppointmentRequestedScreen()),
              ),
            ),
            _buildMetricCard(
              title: 'Appointments Requiring Approval',
            ...
            Row(
              children: [
                ...
                Expanded(
                  child: Text(
                    'Review and manage appointment requests promptly',
                    style: GoogleFonts.poppins(
                      fontSize: 12,
                      fontWeight: FontWeight.w400,
                      color: const Color(0xFF667085),
